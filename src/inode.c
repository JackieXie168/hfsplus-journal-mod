...
#ifdef LINUX_2_6_15
static int hfsplus_journalled_commit_write(struct file *file, struct page *page, unsigned from, unsigned to)
{
	hfsplus_handle_t *hfsplus_handle; 
	struct inode *inode = page->mapping->host;
	int ret = 0, ret2;
	...
	ret = hfsplus_walk_page_buffers(hfsplus_handle, page_buffers(page), from, to, &partial, hfsplus_commit_write_fn);
	if (!partial)
...
static int hfsplus_journalled_prepare_write(struct file *file, struct page *page, unsigned from, unsigned to)
{
	struct inode *inode = page->mapping->host;
	hfsplus_handle_t hfsplus_handle;
	int ret;
	...
	ret = block_prepare_write(page, from, to, hfsplus_get_block);
	if (ret)
		hfsplus_journal_stop(&hfsplus_handle);
			...
			struct page **pagep, void **fsdata)
{
	...
	if((ret = hfsplus_journal_start(__FUNCTION__, inode->i_sb, &hfsplus_handle)))
		...
		*pagep = NULL;
		if((ret = cont_write_begin(file, mapping, pos, len, flags, pagep, fsdata,
					hfsplus_get_block,
					&HFSPLUS_I(mapping->host).phys_size)))
			goto out;
	}
	ret = block_prepare_write(&pagep, from, to, hfsplus_get_block);
	if (ret)
		hfsplus_journal_stop(&hfsplus_handle);
...
#ifdef LINUX_2_6_15
	.prepare_write	= hfsplus_journalled_prepare_write,
	.commit_write = hfsplus_journalled_commit_write,
#else
...
int hfsplus_cat_read_inode(struct inode *inode, struct hfs_find_data *fd)
{
	...
	if (type == HFSPLUS_FOLDER) {
		...
		printk(KERN_ERR "HFS+-fs: bad catalog entry used to create inode\n");
		res = -EIO;
	}
	return res;
}
