...
#ifdef LINUX_2_6_15
	hfsplus_inode_check(struct super_block * sb)(inode->i_sb);
#endif
}
...
static int hfsplus_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!hfsplus_parse_options(data, sbi)) {
		if (!silent)
			printk(KERN_ERR "HFS+-fs: unable to parse mount options\n");
		err = -EINVAL;
		goto cleanup;
	}
	...
	if (hfsplus_read_wrapper(sb)) {
		if (!silent)
			printk(KERN_WARNING "HFS+-fs: unable to find HFS+ superblock\n");
		err = -EINVAL;
		goto cleanup;
	}
	...
	if (be16_to_cpu(vhdr->version) < HFSPLUS_MIN_VERSION ||
		...
		if (!silent)
			printk(KERN_ERR "HFS+-fs: wrong filesystem version\n");
		goto cleanup;
	}
	...
	HFSPLUS_SB(sb).alloc_file = hfsplus_journal_iget(sb, HFSPLUS_ALLOC_CNID);
